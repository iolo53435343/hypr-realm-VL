...
std::string CHyprCtl::getWindowData(PHLWINDOW w, eHyprCtlOutputFormat format) {
    ...
    const auto realm = Realm::windowManager() ? Realm::windowManager()->realmForWindow(w->m_stableID) : SP<Realm::CRealm>{};
    ...
    "stableId": "{:x}"
            ...
            escapeJSONStrings(NContentType::toString(w->getContentType())), (w->m_tearingHint ? "true" : "false"), Realm::realmWindowJSON(realm), w->m_stableID);
    } else {
        return std::format(
            "Window {:x} -> {}:\n\tmapped: {}\n\thidden: {}\n\tvisible: {}\n\tacceptsInput: {}\n\tat: {},{}\n\tsize: {},{}\n\tworkspace: {} ({})\n\tfloating: {}\n\tmonitor: "
            "{}\n\tclass: {}\n\ttitle: "
            ...
            w->xdgDescription().value_or(""), NContentType::toString(w->getContentType()), sc<int>(w->m_tearingHint), Realm::realmWindowText(realm), w->m_stableID);
    }
}
