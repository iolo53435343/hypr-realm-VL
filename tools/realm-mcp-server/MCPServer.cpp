...
static constexpr std::string_view ORCHESTRATOR_TOOLS_RESULT = R"json({"tools":[
  ...
  {"name":"press_shortcut","description":"Atomically press a named key with modifiers in a named realm.","inputSchema":{"type":"object","properties":{"realm":{"type":"string","minLength":1,"maxLength":128},"modifiers":{"type":"array","minItems":1,"maxItems":4,"uniqueItems":true,"items":{"type":"string","enum":["ctrl","alt","shift","meta"]}},"key":{"type":"string","minLength":1,"maxLength":32}},"required":["realm","modifiers","key"],"additionalProperties":false},"annotations":{"title":"Press shortcut in realm","readOnlyHint":false,"destructiveHint":true,"idempotentHint":false,"openWorldHint":false}},
...
std::string CMCPServer::callTool(std::string_view parametersJSON) {
    ...
    if (*parameters.name == "press_shortcut") {
        ...
        keycodes.emplace_back(*keycode);
        return controlResult(m_controlClient.pressShortcut(*realm, keycodes), std::format("Shortcut pressed in realm '{}'.", *realm));
    }
